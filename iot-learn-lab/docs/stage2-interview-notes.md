...
| 场景 | 日期 | 通过？ | 关键现象 |
|------|------|--------|----------|
| K8 Jaeger Feign | 2026-07-23 | ☑ | 同 TraceId；Services=2；`clientName=CommandDispatchClient`；需 `feign-micrometer` |
...
| 问题 | 答法 |
|------|------|
| Trace vs Span？ | Trace=一次请求整条链路；Span=其中一段（HTTP/Feign/Kafka send） |
| 为何 Feign 曾是两条 Trace？ | 未注入 `traceparent`；加 `feign-micrometer` 后同 TraceId |
| `clientName` 是什么？ | Feign 客户端接口（如 `CommandDispatchClient`），`span.kind=client` |
| Kafka 为何常只有 producer？ | 消费在另一进程，靠消息头传播；未串上可记已知限制 |
